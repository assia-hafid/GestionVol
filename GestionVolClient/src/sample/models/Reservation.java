...
public class Reservation {
    ...
    public Ville getVilleDepart(){
        return vol.getVilleDepart();
    }

    public Ville getVilleArrivee(){
        return vol.getVilleArrivee();
    }

    public String getDateDepart(){
        return vol.getDateDepart().toString()+" "+vol.getHeureDepart().toString();
    }

    public String getDateArrive(){
        return vol.getDateArrivee().toString()+" "+vol.getDateArrivee().toString();
    }
...
//        DBConnection dbConnection = new DBConnection();
    ...
    public static ArrayList<Reservation> getClientReservations(int cinClient){
        Connection connection = DBConnection.getInstance().getConnection();
        try {
            PreparedStatement prepared = connection.prepareStatement("select * from reservation where cin_client=? order by date_reservation DESC");
            prepared.setInt(1,cinClient);
            ResultSet resultSet = prepared.executeQuery();
            ArrayList<Reservation> reservations = new ArrayList<>();
            while (resultSet.next()){
                Reservation reservation = new Reservation();
                reservation.setNumVol(resultSet.getInt("num_vol"));
                Vol vol = Vol.getVolByID(resultSet.getInt("num_vol"));
                reservation.setVol(vol);
                reservation.setDateReservation(resultSet.getDate("date_reservation"));
                reservation.setCinClient(cinClient);
                reservations.add(reservation);
            }
            return reservations;
        }catch (Exception e){
            System.out.println("DB Probleme");
            e.printStackTrace();
            return null;
        }
    }
